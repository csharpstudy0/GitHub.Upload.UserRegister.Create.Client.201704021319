...
#        self.__db. = data
    ...
    def Show(self):
        ...
        self.__sum_repo_count = self.__db.repo['Repositories'].count()
        print("リポジトリ総数  : {0}".format(self.__sum_repo_count))
        print("リポジトリ平均数: {0} repo/日".format(self.__sum_repo_count / self.__date_span))
        self.__sum_code_size = self.__db.repo.query("select SUM(Size) SumSize from Languages;").next()['SumSize']
    ...
    def __calc_date(self):
        first_date = self.__db.repo.query("select min(CreatedAt) FirstDate from Repositories;").next()['FirstDate']
        last_date = self.__db.repo.query("select max(CreatedAt) LastDate from Repositories;").next()['LastDate']
    ...
    def __show_sizes_by_languages(self):
        # 桁あわせ：最も長い言語名を取得する
        name_length = 0
        for res in self.__db.repo.query('select * from Languages where length(Language)=(select max(length(Language)) from Languages)'):
            name_length = res['Language']
        # 桁あわせ：最も大きい言語別合計Byteを取得する
        size_length = self.__db.repo.query('select sum(Size) SumSize from Languages group by Language order by SumSize desc').next()['SumSize']
        # 言語別の合計Byte数
        format_str = "  {0:<%d}: {1:>%d} Byte" % (len(name_length), len(str(size_length)))
        for lang in self.__db.repo.query('select Language, sum(Size) SumSize from Languages group by Language order by SumSize desc'):
